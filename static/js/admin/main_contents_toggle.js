...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /**
   * 특정 페이지 표시 함수
   * @param {string} pageKey - 표시할 페이지의 식별자 (pages 객체의 키)
   *
   * 1. 현재 표시된 모든 페이지를 숨김
   * 2. 지정된 페이지만 표시
   */
  ...
  /**
   * 참고사항:
   * - 이 스크립트는 SPA(Single Page Application) 방식으로 동작
   * - 실제 페이지 이동 없이 컨텐츠 영역만 교체
   * - display 속성을 통한 간단한 토글 방식 사용
   * - Object.values()와 Object.entries()를 활용한 객체 순회
   * - 이벤트 위임(delegation) 패턴은 사용하지 않음
   */
});
